...
//Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
		...
		txtValor.setBounds(150, 328, 100, 33);
	...
	public void guardarReservas() {
		...
		Reservas reserva = new Reservas(java.sql.Date.valueOf(fechaEntrada), java.sql.Date.valueOf(fechaSalida),
	...
	public String generaNumeroReserva() {
		int numero = (int) Math.floor(Math.random() * 1000 + 1);
		String numeroS = String.valueOf(numero);
		System.out.println("numero reserva " + numeroS);
		return numeroS;
	}
 ...
 * 
 * String fechaEntrada =
 * ((JTextField)txtFechaE.getDateEditor().getUiComponent()).getText(); String
 * fechaSalida =
 * ((JTextField)txtFechaS.getDateEditor().getUiComponent()).getText(); int
 * totalDias = 0;
 * 
 * SimpleDateFormat formatoFecha = new SimpleDateFormat("yyyy-MM-dd");
 * 
 * 
 * try { Date fechaFin = formatoFecha.parse(fechaSalida); Date fechaInicio =
 * formatoFecha.parse(fechaEntrada); long tiempoTranscurrido =
 * fechaFin.getTime() - fechaInicio.getTime(); TimeUnit unidad = TimeUnit.DAYS;
 * totalDias = (int) unidad.convert(tiempoTranscurrido, TimeUnit.MILLISECONDS);
 * 
 * 
 * } catch (ParseException e) { e.printStackTrace(); }
 * 
 * System.out.println("total dias"+totalDias); return totalDias;
 * 
 * }
 * 
 * public String valorReserva() {
 * 
 * double precioPorNoche = 1250.75; double totalPorNoche = 0;
 * 
 * totalPorNoche = calcularDiasReserva()*precioPorNoche; String totalPorNocheS =
 * String.valueOf(totalPorNoche);
 * 
 * System.out.println("precio total"+totalPorNoche);
 * 
 * return totalPorNocheS; }
 */
